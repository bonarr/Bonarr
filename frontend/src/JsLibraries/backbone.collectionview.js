  ...
  Backbone.CollectionView = Backbone.View.extend({
    ...
    setOption: function(name, value) {
      ...
      if (name === "collection") {
        ...
        if (_.contains(kAllowedOptions, name)) {
          ...
          throw name + " is not an allowed option";
      }
    },
      ...
      switch (referenceBy) {
        ...
        case "offset" :
          ...
          if (this._isRenderedAsTable())
            itemElements = this.$el.find("> tbody > [data-model-cid]:not(.not-visible)");else if (this._isRenderedAsList())
    ...
    _getClickedItemId: function(theEvent) {
      ...
      if (clickedItem.length > 0) {
        ...
        if ($.isNumeric(clickedItemId))
          clickedItemId = parseInt(clickedItemId, 10);
      }
    ...
    _updateItemTemplate: function() {
      ...
      if (itemTemplateHtml)
        this.itemTemplateFunction = _.template(itemTemplateHtml);
    },
    ...
    _addSelectedClassToSelectedItems: function(oldItemsIdsWithSelectedClass) {
      if (_.isUndefined(oldItemsIdsWithSelectedClass))
    ...
    _createNewModelView: function(model, modelViewOptions) {
      ...
      if (_.isUndefined(modelViewConstructor))
